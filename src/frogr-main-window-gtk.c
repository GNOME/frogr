/*
 * frogr-main-window-gtk.c -- Gtk main window for the application
 *
...
#define MAIN_WINDOW_GTK_ICON(_s) ICONS_DIR "/hicolor/" _s "/apps/frogr.png"

#define FROGR_MAIN_WINDOW_GTK_GET_PRIVATE(object)                   \
  (G_TYPE_INSTANCE_GET_PRIVATE ((object),                       \
                                FROGR_TYPE_MAIN_WINDOW_GTK,         \
                                FrogrMainWindowGtkPrivate))

typedef struct _FrogrMainWindowGtkPrivate {
  GtkWidget *menu_bar;
  GtkWidget *icon_view;
...
G_DEFINE_TYPE (FrogrMainWindowGtk, frogr_main_window_gtk, FROGR_TYPE_MAIN_WINDOW);
...
static void _populate_menu_bar (FrogrMainWindowGtk *self);
static GtkWidget *_ctxt_menu_create (FrogrMainWindowGtk *self);
...
static void _initialize_drag_n_drop (FrogrMainWindowGtk *self);
...
_update_ui (FrogrMainWindow *self)
{
  ...
  switch (mw_priv->state)
    ...
    case FROGR_STATE_IDLE:
      npics = frogr_main_window_model_n_pictures (mw_priv->model);
      pictures_loaded = frogr_main_window_model_n_pictures (mw_priv->model);
...
static void
_set_status_text (FrogrMainWindow *self,
                                   const gchar *text)
{
  g_return_if_fail(FROGR_IS_MAIN_WINDOW_GTK (self));
...
static void
_set_progress (FrogrMainWindow *self,
                                double fraction,
                                const gchar *text)
{
  g_return_if_fail(FROGR_IS_MAIN_WINDOW_GTK (self));
...
static void
_populate_menu_bar (FrogrMainWindowGtk *self)
{
  FrogrMainWindowGtkPrivate *priv = FROGR_MAIN_WINDOW_GTK_GET_PRIVATE (self);
...
static GtkWidget *
_ctxt_menu_create (FrogrMainWindowGtk *self)
{
...
{
  FrogrMainWindow *mainwin = FROGR_MAIN_WINDOW (data);
  _frogr_main_window_add_tags_to_pictures (mainwin);
}
...
{
  FrogrMainWindow *mainwin = FROGR_MAIN_WINDOW (data);
  _frogr_main_window_edit_selected_pictures (mainwin);
}
...
{
  FrogrMainWindow *mainwin = FROGR_MAIN_WINDOW (data);
  _frogr_main_window_remove_selected_pictures (mainwin);
}

static void
_initialize_drag_n_drop (FrogrMainWindowGtk *self)
{
  FrogrMainWindowGtkPrivate *priv = FROGR_MAIN_WINDOW_GTK_GET_PRIVATE (self);
                                  ...
                                  gpointer data)
{
  ...
  if (filepaths_list != NULL)
    _frogr_main_window_load_pictures (FROGR_MAIN_WINDOW (self), filepaths_list);
  ...
  if (gtk_dialog_run (GTK_DIALOG (dialog)) == GTK_RESPONSE_ACCEPT)
    {
      ...
      if (filepaths != NULL)
        {
          _frogr_main_window_load_pictures (FROGR_MAIN_WINDOW (self), filepaths);
...
{
  FrogrMainWindow *mainwin = FROGR_MAIN_WINDOW (data);
  _frogr_main_window_upload_pictures (mainwin);
}
...
{
  FrogrMainWindow *mainwin = FROGR_MAIN_WINDOW (data);
  FrogrMainWindowGtkPrivate *priv = FROGR_MAIN_WINDOW_GTK_GET_PRIVATE (data);
  FrogrMainWindowPrivate *mw_priv = FROGR_MAIN_WINDOW_GET_PRIVATE (data);
  GtkTreePath *path;
  ...
  if (gtk_icon_view_get_item_at_pos (GTK_ICON_VIEW (priv->icon_view),
                                     ...
                                     NULL))
    {
      ...
      if (mw_priv->state == FROGR_STATE_IDLE)
        {
...
{
  FrogrMainWindowPrivate *mw_priv = FROGR_MAIN_WINDOW_GET_PRIVATE (self);
  frogr_controller_quit_app (mw_priv->controller);
}
...
{
  FrogrMainWindowGtk *self = FROGR_MAIN_WINDOW_GTK (object);
  FrogrMainWindowGtkPrivate *priv = FROGR_MAIN_WINDOW_GTK_GET_PRIVATE (self);
  ...
  G_OBJECT_CLASS(frogr_main_window_gtk_parent_class)->finalize (object);
}

static void
frogr_main_window_gtk_class_init (FrogrMainWindowGtkClass *klass)
{
  GObjectClass *obj_class = (GObjectClass *)klass;
  FrogrMainWindowClass *mainwin_class = FROGR_MAIN_WINDOW_CLASS (klass);

  obj_class->finalize = _frogr_main_window_gtk_finalize;

  mainwin_class->update_ui = _update_ui;
  mainwin_class->get_selected_pictures = _get_selected_pictures;
  mainwin_class->add_picture_to_ui = _add_picture_to_ui;
  mainwin_class->remove_pictures_from_ui = _remove_pictures_from_ui;
  mainwin_class->set_status_text = _set_status_text;
  mainwin_class->set_progress = _set_progress;

  g_type_class_add_private (obj_class, sizeof (FrogrMainWindowGtkPrivate));
}
